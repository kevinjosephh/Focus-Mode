chrome.storage.sync.get(["focusMode", "blockedSites"], function (data) {
  if (data.focusMode && data.blockedSites) {
    ...
    const baseCurrentSite = currentSite.replace(/^www\./, ""); // Normalize current site

    // Check if current site or its variations are blocked
    ...
    if (isBlocked) {
      ...
      blockMessage.style.backgroundColor = "rgba(255, 255, 255, 0.9)";
      ...
      blockMessage.style.pointerEvents = "all";
